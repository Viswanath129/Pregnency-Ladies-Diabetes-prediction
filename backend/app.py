...
async def predict_risk(request: Request, data: PatientVitals):
    try:
        ...
        if request.app.state.models_loaded:
            # 1. Use DataFrame for the scaler as it likely has feature names
            df = pd.DataFrame([vitals], columns=cols)
            scaled_data = request.app.state.models["scaler"].transform(df)

            # 2. Convert to raw numpy array for models fitted without feature names
            # to suppress Scikit-Learn UserWarnings
            scaled_data_np = np.asarray(scaled_data)

            # Get probabilities from individual streams
            p_ml = request.app.state.models["ml"].predict_proba(scaled_data_np)[:, 1][0]
            
            # ANN prediction (Handling potential different formats)
            try:
                p_ann = request.app.state.models["ann"].predict_proba(scaled_data_np)[:, 1][0]
            except:
                pred = request.app.state.models["ann"].predict(scaled_data_np)
            ...
            # meta_input as raw numpy array to avoid feature name warnings if fitted without them
            meta_input = np.array([[p_ml, p_ann, p_sv]])
            final_prob = request.app.state.models["meta"].predict_proba(meta_input)[:, 1][0]
            is_sim = False
        else:
