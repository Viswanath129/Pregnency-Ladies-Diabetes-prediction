...
def predict_risk(data: PatientVitals):
    # Performance Optimization: Using standard `def` instead of `async def` for this CPU-bound
    # ML inference task allows FastAPI to run it in a threadpool, preventing event loop blocking.
    try:
        ...
        if MODELS_LOADED:
            # Performance Optimization: Bypass pandas.DataFrame instantiation overhead for single-row inference
