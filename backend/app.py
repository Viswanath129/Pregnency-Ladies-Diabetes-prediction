...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", UserWarning)
                final_prob = MODELS["meta"].predict_proba(np.array([[p_ml, p_ann, p_q]]))[:, 1][0]
            is_sim = False
        else:
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    # Ensure they are native floats to prevent serialization issues and standardize types
    ...
    # Calculate stddev manually for list of length 3 for performance
