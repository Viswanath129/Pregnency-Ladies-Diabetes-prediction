    ...
    try:
        ...
        if MODELS_LOADED:
            with warnings.catch_warnings():
                ...
                # Performance Optimization: Bypass pandas DataFrames for single-row inference
                # and use 2D numpy arrays instead.
                vitals_arr = np.array([vitals])
                scaled_data = MODELS["scaler"].transform(vitals_arr)

                # Get probabilities from individual streams, cast to standard float
