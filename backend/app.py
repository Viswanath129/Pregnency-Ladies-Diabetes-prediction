...
def predict_risk(data: PatientVitals):
    # Viswa: Changed async def to def. CPU-bound sklearn inference blocks the main
    # event loop if marked async. A standard def runs in FastAPI's external threadpool.
    try:
        ...
        if MODELS_LOADED:
            # Viswa: Replaced pd.DataFrame with 2D np.array for single-row inference.
            # This avoids significant Pandas instantiation/validation overhead (~8x speedup).
