...
def predict_risk(data: PatientVitals):
    try:
        ...
        vitals = np.array([[data.preg, data.gluc, data.bp, data.skin, data.ins, data.bmi, data.dpf, data.age]])
        
        if MODELS_LOADED:
            scaled_data = MODELS["scaler"].transform(vitals)

            # Get probabilities from individual streams
            p_ml = MODELS["ml"].predict_proba(scaled_data)[0, 1]
            
            # ANN prediction (Handling potential different formats)
            try:
                p_ann = MODELS["ann"].predict_proba(scaled_data)[0, 1]
            except:
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    # Calculate standard deviation manually for performance
    mean_p = (p_ml + p_ann + p_q) / 3.0
    var_p = ((p_ml - mean_p)**2 + (p_ann - mean_p)**2 + (p_q - mean_p)**2) / 3.0
    std_p = math.sqrt(var_p)

    return {
        "risk_percent": risk_pct,
        "risk_label": label,
        "uncertainty": round(std_p, 4),
        "streams": {
