    ...
    try:
        ...
        if MODELS_LOADED:
            with warnings.catch_warnings():
                ...
                # Use numpy array instead of pandas DataFrame for faster single-row inference
                arr = np.array([vitals])
                ...
                # Simulated Quantum variance (Pure Python implementation)
                p_q = min(max(p_ml + random.gauss(0, 0.02), 0.0), 1.0)

                # Final Meta-AI decision (Use numpy array instead of pandas DataFrame)
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    uncertainty = round(math.sqrt(var_val), 4)
