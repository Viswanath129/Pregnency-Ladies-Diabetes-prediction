...
# Ignore warnings about feature names when using numpy arrays instead of DataFrames
...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            ...
            p_q = min(max(p_ml + random.gauss(0, 0.02), 0), 1)
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    return {
        ...
        "risk_label": label,
        "uncertainty": round(float(((p_ml - (p_ml + p_ann + p_q) / 3) ** 2 + (p_ann - (p_ml + p_ann + p_q) / 3) ** 2 + (p_q - (p_ml + p_ann + p_q) / 3) ** 2) / 3) ** 0.5, 4),
        "streams": {
