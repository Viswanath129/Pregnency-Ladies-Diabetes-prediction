...
async def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            # OPTIMIZATION: Avoid pandas DataFrame instantiation overhead for single-row inference.
            # Using 2D numpy array is ~20x faster. We suppress the UserWarning about missing feature names.
            with warnings.catch_warnings():
                ...
                try:
                    p_ann = float(
                        MODELS["ann"].predict_proba(scaled_data)[0, 1]
                    )
                except Exception:
                    pred = MODELS["ann"].predict(scaled_data)
                    p_ann = float(
                        pred[0][0] if len(pred.shape) > 1 else pred[0]
                    )
                ...
                final_prob = float(
                    MODELS["meta"].predict_proba(meta_input)[0, 1]
                )
            is_sim = False
        else:
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    label = "High" if risk_pct > 70 else (
        "Moderate" if risk_pct > 40 else "Low"
    )
