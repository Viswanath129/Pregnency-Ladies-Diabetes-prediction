...
def predict_risk(data: PatientVitals):
    try:
        ...
        vitals_arr = np.array([[data.preg, data.gluc, data.bp, data.skin, data.ins, data.bmi, data.dpf, data.age]])
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    # Calculate standard deviation with pure Python
    arr = [p_ml, p_ann, p_q]
    mean = sum(arr) / 3
    variance = sum((x - mean) ** 2 for x in arr) / 3
