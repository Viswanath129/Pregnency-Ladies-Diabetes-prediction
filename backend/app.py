...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            # OPTIMIZATION: Bypass pd.DataFrame for single row inference and use numpy directly to save overhead.
            # Catch warnings about missing feature names (which is expected when bypassing DataFrame).
            arr = np.array([vitals])
