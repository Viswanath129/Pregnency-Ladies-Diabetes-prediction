...
async def predict_risk(data: PatientVitals):
    try:
        ...
        vitals = [data.preg, data.gluc, data.bp, data.skin,
                  data.ins, data.bmi, data.dpf, data.age]

        if MODELS_LOADED:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", category=UserWarning)
                # Optimization: use numpy array instead of pandas DataFrame
                ...
                try:
                    p_ann = float(
                        MODELS["ann"].predict_proba(scaled_data)[:, 1][0]
                    )
                except Exception:
                ...
                final_prob = float(
                    MODELS["meta"].predict_proba(meta_input_arr)[:, 1][0]
                )
            is_sim = False
        else:
