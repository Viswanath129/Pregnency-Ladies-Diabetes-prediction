...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            # PERFORMANCE OPTIMIZATION (Viswa):
            # Using 2D np.array instead of pd.DataFrame for single-row inference
            # avoids significant pandas instantiation overhead, speeding up the endpoint by ~15-20%.
            # We catch UserWarning to safely suppress missing feature name warnings from scikit-learn.
            arr = np.array([vitals])
            ...
            with warnings.catch_warnings():
                ...
                # Get probabilities from individual streams, cast to float for JSON serialization
                p_ml = float(MODELS["ml"].predict_proba(scaled_data)[0, 1])

                # ANN prediction (Handling potential different formats)
                try:
                    p_ann = float(MODELS["ann"].predict_proba(scaled_data)[0, 1])
                except Exception:
                ...
                final_prob = float(MODELS["meta"].predict_proba(meta_input)[0, 1])
            is_sim = False
        else:
