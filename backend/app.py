...
FILES = {
    ...
    "meta": "ensemble_engine_decision.joblib",
...
async def lifespan(app: FastAPI):
    """
    Lifespan context manager for robust model initialization and resource management.
    """
    global MODELS
    try:
...
async def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS:
            # Pass input data as a DataFrame with correct feature names to align with training
            ...
            # Note: We use a raw numpy array for the meta model as well
...
def build_response(final_prob, p_ml, p_ann, p_sv, is_sim):
    ...
    return {
        ...
        "streams": {
            ...
            "stochastic variance": round(p_sv * 100, 2)
        },
        "is_simulated": is_sim
    }
