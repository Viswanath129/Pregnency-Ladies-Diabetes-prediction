...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            # Bypass DataFrame instantiation and use 2D numpy array
            with warnings.catch_warnings():
                ...
                # Simulated Quantum variance using pure python instead of np.clip / np.random.normal
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    # Calculate std manually using pure python
