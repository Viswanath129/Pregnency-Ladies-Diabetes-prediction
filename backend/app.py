    ...
    try:
        ...
        if MODELS_LOADED:
            with warnings.catch_warnings():
                ...
                # OPTIMIZATION: Use 2D numpy array instead of pandas DataFrame for faster single-row inference
                arr = np.array([vitals])
                ...
                # OPTIMIZATION: Simulated Quantum variance using pure Python for scalars instead of np.clip/np.random.normal
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    # OPTIMIZATION: Pure Python standard deviation calculation instead of np.std
