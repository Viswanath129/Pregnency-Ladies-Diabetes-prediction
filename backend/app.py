...
def predict_risk(data: PatientVitals):
    try:
        ...
        if MODELS_LOADED:
            # Bypass DataFrame creation for single-row inference for performance
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", UserWarning)
                arr_vitals = np.array([vitals])
                scaled_data = MODELS["scaler"].transform(arr_vitals)
                ...
                arr_meta = np.array([[p_ml, p_ann, p_q]])
                final_prob = float(MODELS["meta"].predict_proba(arr_meta)[:, 1][0])
            is_sim = False
        else:
            # Mathematical Simulation fallback
            final_prob = float((data.gluc / 300) * 0.7 + (data.bmi / 50) * 0.3)
            p_ml, p_ann, p_q = float(final_prob * 0.9), float(final_prob * 1.1), float(final_prob)
            is_sim = True
