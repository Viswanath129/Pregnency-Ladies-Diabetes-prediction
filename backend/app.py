    ...
    try:
        ...
        if MODELS_LOADED:
            ...
            with warnings.catch_warnings():
                ...
                # Simulated Quantum variance (Pure Python math)
                p_q = float(min(1.0, max(0.0, p_ml + random.gauss(0, 0.02))))
...
def build_response(final_prob, p_ml, p_ann, p_q, is_sim):
    ...
    # Calculate standard deviation in pure Python
    mean_val = (p_ml + p_ann + p_q) / 3.0
    variance = ((p_ml - mean_val)**2 + (p_ann - mean_val)**2 + (p_q - mean_val)**2) / 3.0
    std_dev = variance**0.5

    return {
        "risk_percent": risk_pct,
        "risk_label": label,
        "uncertainty": round(float(std_dev), 4),
        "streams": {
